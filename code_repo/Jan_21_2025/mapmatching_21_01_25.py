...
matched_tracks = MapMatchTracks(
    ...
    radius_m=10,          # Match radius in meters
...
# %%
# Convert filter_xy_tuple to dictionary
filter_xy_dict = {f"filter_point{i+1}": coord for i, coord in enumerate(filter_xy_tuple)}

# Convert match_xy_tuple to dictionary
match_xy_dict = {f"match_point{i+1}": coord for i, coord in enumerate(match_xy_tuple)}

print("Filter Coordinates Dictionary:")
print(filter_xy_dict)
...
def count_decimal_places(num):
    ...
    num_str = f"{num:.100f}".rstrip('0')  # Ensure trailing zeros are removed
    if '.' in num_str:
        return len(num_str.split('.')[1])  # Count characters after the decimal point
    return 0  # If no decimal point, return 0



# Extract filter_xy_tuple
coordinates_extracted = matched_tracks[0]["track_id"]["filter_xy_tuple"]

# Count decimal places for each number in the tuples
decimal_places = [
    [count_decimal_places(coord) for coord in coordinate_pair]
    for coordinate_pair in coordinates_extracted
]

print("Decimal places in the tuples:", decimal_places)
# %%
# Example matched_tracks structure with tuple of coordinates

# If you want to confirm the precision, let's print the tuple:
coordinates_raw_matched = matched_tracks[0]["track_id"]["match_xy_tuple"]

# You can access the tuples and store them with full precision in the dictionary
matched_tracks_dict = {
    "match_xy_tuple": [(float(coord[0]), float(coord[1])) for coord in coordinates_raw_matched]
}

# Verify that decimal places are intact
for coord in matched_tracks_dict["match_xy_tuple"]:
    print(coord)
...
# Example coordinates to store


# Extract the coordinates and convert to Decimal
match_xy_tuple = track_id['match_xy_tuple']
match_xy_tuple_decimal = [(Decimal(coord[0]), Decimal(coord[1])) for coord in match_xy_tuple]

# Prepare the data for export with 50 decimal places
formatted_data = [(f"{coord[0]:.100f}", f"{coord[1]:.100f}") for coord in match_xy_tuple_decimal]

# Specify the CSV file path
csv_file_path = '/Users/kad/Desktop/cyclists/Kimberley/segmentation/troubleshoot/50_dp/match_xy_tuple_100.csv'

# Write the data to CSV
with open(csv_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Longitude', 'Latitude'])  # Column headers
...
gdf.to_csv('/Users/kad/Desktop/cyclists/Kimberley/segmentation/troubleshoot/matched_xy_r10.csv')
gdf_filter.to_csv('/Users/kad/Desktop/cyclists/Kimberley/segmentation/troubleshoot/filter_xy_r10.csv')
gdf_all.to_csv('/Users/kad/Desktop/cyclists/Kimberley/segmentation/troubleshoot/mapmatched_r10.csv')
# %% Exporting the dictionary so it can be used for segmentation
import pickle
with open('/Users/kad/Desktop/cyclists/Kimberley/segmentation/troubleshoot/matched_dict_tracksMaike_23-11-23_r10', 'wb') as file:
