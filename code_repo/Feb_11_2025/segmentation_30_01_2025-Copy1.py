...
df_ngapipi_data = {
    ...
    'match_xy_tuple': ngapipi_data['track_id']['match_xy_tuple'],
...
sample_edge = list(edge_directions.edges(data=True))[225]  # Get the first edge
print(sample_edge)
...
print("Sample edge attributes:")
segment_edges_graph_sample_edge = list(segment_edges_graph.edges(data=True))[225]  # Get the first edge
...
print("Sample nodes:", list(GRAPH_cumulative_segmentation.nodes)[:20])  
print("Sample edges:", list(GRAPH_cumulative_segmentation.edges(data=True))[:20])
...
for road, edges in cumulative_segmentation_graph.items():
    ...
    print(f"Edges: {edges[:20]}")  # Print first 5 edges for debugging
...
get_graph = GetGraph(
    graph_path='/Users/kad/Desktop/cyclists/Kimberley/segmentation/jupyter/topologic_Graph_10_feb_V2.gpkg',  # Ensure it's a file, not a directory
    segment_length=5, 
    segment_method='edge', 
    area_name='allakl', 
    ...
    debug=True, 
    way_group_attribute='seg_group'  # Replace 'name' with the actual attribute you're grouping by
)
...
# Assign a CRS to the graph if it's missing
if "crs" not in GRAPH_cumulative_segmentation.graph:
    GRAPH_cumulative_segmentation.graph["crs"] = "EPSG:4326"  # Or any appropriate CRS (e.g., WGS84)
...
def GetRideSegsGDF(matched_gdf, matched_filename, network_graph, seg_multi_gdf, 
    ...
    track_segs = GetSegsAlongTracks({matched_filename: matched_gdf}, network_graph, max_pathlength)
    print("Keys in track_segs:", track_segs.keys())  # Should contain matched_filename

    track_segs_graph = track_segs.get(matched_filename, {}).get('sp_graph', None)
    print("Type of track_segs_graph:", type(track_segs_graph))  # Should be networkx graph

    if track_segs_graph:
        print("Example edges:", list(track_segs_graph.edges(data=True))[:5])  # Inspect attributes

    track_seg_ids = [i[2].get(group_attr, 'MISSING') for i in track_segs_graph.edges(data=True)]
    print("Segment IDs:", track_seg_ids[:10])  # Should show segment IDs, not 'MISSING'

    ride_segments = GetRideSegsGDF(ngapipi_gdf, matched_filename, Ordered_seg_graph, Segment_gdf)
    print(ride_segments.head())  # Ensure output is as expected
...
ride_segments = GetRideSegsGDF(ngapipi_gdf,'match_xy_tuple',Ordered_seg_graph,Segment_gdf)
ride_segments
...
# Accessing the latitude and longitude of node 25842600
latitude_25842600 = Ordered_seg_graph.nodes[25842600]['y']
longitude_25842600 = Ordered_seg_graph.nodes[25842600]['x']

# Accessing the latitude and longitude of node 9882971783
latitude_9882971783 = Ordered_seg_graph.nodes[9882971783]['y']
longitude_9882971783 = Ordered_seg_graph.nodes[9882971783]['x']

print(f"Node 25842600 coordinates: Latitude = {latitude_25842600}, Longitude = {longitude_25842600}")
print(f"Node 9882971783 coordinates: Latitude = {latitude_9882971783}, Longitude = {longitude_9882971783}")
...
# Create a folium map centered around the midpoint of the two nodes
m = folium.Map(location=[(latitude_25842600 + latitude_9882971783) / 2, 
                         (longitude_25842600 + longitude_9882971783) / 2], zoom_start=15)

# Add markers for both nodes
folium.Marker([latitude_25842600, longitude_25842600], 
              popup="Node 25842600", icon=folium.Icon(color='red')).add_to(m)

folium.Marker([latitude_9882971783, longitude_9882971783], 
              popup="Node 9882971783", icon=folium.Icon(color='blue')).add_to(m)

# Save the map to an HTML file to view interactively
m
...
def SegGDFFromGraph(graph:nx.MultiDiGraph):
    rows = []
    for index, (u, v, data) in enumerate(tqdm(graph.edges(data=True))):
        for start_xy, end_xy, seg_id, seg_uid, seg_grp, length in data['segments']:  # Added seg_grp
            rows.append({
                "geometry": LineString([Point(start_xy), Point(end_xy)]),
                "seg_id": seg_id,
                "seg_uid": seg_uid,
                "seg_group": seg_grp,  # Restored seg_group
                "seg_length": length,
                "osmid": data["osmid"]
            })
...
edges = Ordered_seg_graph.edges(data=True)
edge_attrs = set(key for _, _, data in edges for key in data.keys())
...
# Get a list of node attributes
nodes = Ordered_seg_graph.nodes(data=True)
node_attrs = set(key for _, data in nodes for key in data.keys())
...
edges_gdf = ox.graph_to_gdfs(Ordered_seg_graph, nodes=False)

# Check the first few rows
print(edges_gdf.head())

# Plot edges with different colors for each segment (if 'segment_group' exists)
if 'segment_group' in edges_gdf.columns:
    fig, ax = plt.subplots(figsize=(10, 10))
    edges_gdf.plot(column='segment_group', cmap='viridis', linewidth=1, ax=ax, legend=True)
    plt.show()
else:
    print("No 'segment_group' attribute found.")


# 
# ## code for interpolating attribute values between points
# 
# either temporally or spatially
# - both methods will result in a set of points
# - spatial segments will have points within a line, but these points will still be needed to interpolate attribute values
# 
# 
...
#Interpolation modules/packages
import copy
import pandas as pd
import numpy as np
import geopandas as gpd
...
# used for kriging
from pykrige import UniversalKriging, OrdinaryKriging
...
def GetLineCentroids(segments:gpd.GeoDataFrame, return_shapely:bool=False,
    ...
    for geom in list(segments['geometry']):
        # I get the feeling start/end means nothing here, they are just the edges, not in order
        if len(geom.boundary) == 0:
...
ride_segments.geom_type.unique()
...
def GetLineCentroids(segments: gpd.GeoDataFrame, return_shapely: bool = False, return_gdf: bool = True):
    if segments.empty:
        print("Warning: Input GeoDataFrame is empty!")
        return gpd.GeoDataFrame(columns=['seg_uid', 'start', 'end', 'centroid']) if return_gdf else ([], [], [], [])
    ...
    for geom in segments['geometry']:
        if not isinstance(geom, LineString) or geom.is_empty:
            print(f"Skipping invalid geometry: {geom}")
            start, end, centroid = None, None, None
        else:
            # Correctly handle MultiPoint boundary
            if isinstance(geom.boundary, MultiPoint):
                start, end = list(geom.boundary.geoms)  # Extract individual points
            else:
                start, end = geom.boundary

            centroid = geom.centroid
        
        start_points.append(start if return_shapely else (start.x, start.y) if start else None)
        end_points.append(end if return_shapely else (end.x, end.y) if end else None)
        centroids.append(centroid if return_shapely else (centroid.x, centroid.y) if centroid else None)
...
line_centroids = GetLineCentroids(ride_segments) # not working 11/02/25
...
Line_string_centroids.plot()


# In[124]:


Line_string_centroids
...
ngapipi_data_no2 = {
    'track_id': {
        ...
        'match_distance': [10.62806, 5.325484, 0, 0, 12.858593, 0, 0, 0],
        'no2':[3, 4.5, 7, 1, 3, 4.5, 7, 1,]
    }
}

# Convert dictionary to DataFrame
df_ngapipi_data_no2 = pd.DataFrame({
    'filter_xy': ngapipi_data_no2['track_id']['filter_xy_tuple'],
    'match_xy_tuple': ngapipi_data_no2['track_id']['match_xy_tuple'],
    'match_geometry': ngapipi_data_no2['track_id']['match_geometry'],
    'match_roadname': ngapipi_data_no2['track_id']['match_roadname'],
    'match_distance': ngapipi_data_no2['track_id']['match_distance'],
    'no2': ngapipi_data_no2['track_id']['no2']
})

# Extract latitude and longitude correctly
df_ngapipi_data_no2[['long', 'lat']] = pd.DataFrame(df_ngapipi_data_no2['match_xy_tuple'].to_list(), index=df_ngapipi_data_no2.index)

# Create geometry column with correctly named columns
df_ngapipi_data_no2['map_matched_geom'] = df_ngapipi_data_no2.apply(lambda row: Point(row['long'], row['lat']), axis=1)

# Convert to a GeoDataFrame
ngapipi_gdf_no2 = gpd.GeoDataFrame(df_ngapipi_data_no2, geometry='map_matched_geom')

# Set CRS to WGS 84 (EPSG:4326)
ngapipi_gdf_no2.set_crs(epsg=4326, inplace=True)
...
# Voronoi (spatial join to nearest)
def Voronoi(track, tracksegs_gdf, pol_columns, agg_attr='seg_group'):
    # spatial join
    joined = gpd.sjoin_nearest(tracksegs_gdf, track, distance_col='join_distance')

    # rename columns
    left_colname = agg_attr + '_left'
    # have to do a check for _left suffix if both inputs have a seg_uid col
    cols = ([left_colname if left_colname in joined.columns else agg_attr] + pol_columns)
    joined = joined[cols]
    if left_colname in joined.columns:
        cnames = {left_colname: agg_attr}
    else:
        cnames = {agg_attr: agg_attr}
    for col in pol_columns:
        cnames[col] = 'voronoi_' + col
    joined = joined.rename(columns=cnames)
    
    return joined

voronoi = Voronoi(ngapipi_gdf_no2,Line_string_centroids, ["no2"])
...
# taken from https://stackoverflow.com/questions/3104781/inverse-distance-weighted-idw-interpolation-with-python/3114117#3114117
#   and https://gist.github.com/Majramos/5e8985adc467b80cccb0cc22d140634e
def IDW(track, tracksegs_gdf, agg_attr='seg_group', cnt_attr='line_string_centroid',
        pol_attr:str='no2', power=1):
    
    track['x'] = track['map_matched_geom'].x
    track['y'] = track['map_matched_geom'].y
    # get relevant data
    x, y, z = track['x'], track['y'], track[pol_attr]

    # get centroids
    xi, yi = tracksegs_gdf[cnt_attr].x, tracksegs_gdf[cnt_attr].y 

    # build distance matrix (euclidean) between observations and interpretations
    #   in this case, obs=GPX points, interp=segment centroids
    #   see http://stackoverflow.com/questions/1871536
    obs = np.vstack((x, y)).T
    ...
    idw_vals = np.dot(weights.T, z)

    # return 
    df = pd.DataFrame([tracksegs_gdf[agg_attr], idw_vals]).T
    df.columns = [agg_attr, f'idw_{pol_attr}']
    df[f'idw_{pol_attr}'] = pd.to_numeric(df[f'idw_{pol_attr}'])

    return df

IDW_output = IDW(ngapipi_gdf_no2, Line_string_centroids)
...
# Kriging 
def Kriging(track, tracksegs_gdf, agg_attr='seg_group', cnt_attr='line_string_centroid', pol_attr:str='no2', 
            method='ordinary', model='linear'):
    ...
    if method == 'ordinary':
        krigingobj = OrdinaryKriging(x, y, z, model, pseudo_inv=True, coordinates_type='geographic')
    elif method == 'universal':
        krigingobj = UniversalKriging(x, y, z, model, pseudo_inv=True, coordinates_type='geographic')

    # execute kriging
    zi, variance = krigingobj.execute('points', xi, yi)

    # return
    df = pd.DataFrame([tracksegs_gdf[agg_attr], zi]).T
    df.columns = [agg_attr, f'krig_{pol_attr}']
    df[f'krig_{pol_attr}'] = pd.to_numeric(df[f'krig_{pol_attr}'])
    
    return df, variance

Kriging_output = (ngapipi_gdf_no2, Line_string_centroids)
df_output, _ = Kriging(ngapipi_gdf_no2, Line_string_centroids)
_, variance_output = Kriging(ngapipi_gdf_no2, Line_string_centroids)
...
def IDW_SearchRadius(track, tracksegs_gdf, agg_attr='seg_group', cnt_attr='line_string_centroid', 
                     pol_attr:str='no2', search_radius=0.02, power=1):
    import numpy as np
    from sklearn.neighbors import NearestNeighbors

    target = gpd.GeoDataFrame(tracksegs_gdf)
    target_points = np.column_stack((target[cnt_attr].x, target[cnt_attr].y))

    data_pts = track
    x, y, z = data_pts['x'], data_pts['y'], data_pts[pol_attr]
    data_points = np.column_stack((x, y))
    values = z

    # Find neighboring points within the search radius for each target point
    neigh = NearestNeighbors(radius=search_radius)
    neigh.fit(data_points)
    distances, indices = neigh.radius_neighbors(target_points)
    ...
    for i in range(len(target_points)):
        ...
        if len(neighbor_indices) > 0:
            ...
            weights = 1 / np.power(neighbor_distances, power)
            interpolated_value = np.average(neighbor_values, weights=weights)
        else:
    ...
    # return
    df = pd.DataFrame([tracksegs_gdf[agg_attr], interpolated_values]).T
    df.columns = [agg_attr, f'idw_{pol_attr}']
    df[f'idw_{pol_attr}'] = pd.to_numeric(df[f'idw_{pol_attr}'])
    
    return df

IDW_search_radius = IDW_SearchRadius(ngapipi_gdf_no2, Line_string_centroids)
...
def All(data_gdf, seg_gdf, pol_attr:str='no2', agg_attr = 'seg_group', cnt_attr = 'line_string_centroid', krig_method='ordinary', 
        krig_model='linear', idw_power=1, use_searchradius=False, idw_searchradius=0.05):

    voronoi_df = Voronoi(track=data_gdf,
                         tracksegs_gdf=seg_gdf,
                         pol_columns=[pol_attr],
                         agg_attr=agg_attr)
    
    if not use_searchradius:
        idw_df = IDW(track=data_gdf, 
                    tracksegs_gdf=seg_gdf, 
                    agg_attr=agg_attr,
                    cnt_attr=cnt_attr,
                    pol_attr=pol_attr, 
        ...
        idw_df = IDW_SearchRadius(track=data_gdf,
                                  tracksegs_gdf=seg_gdf,
                                  agg_attr=agg_attr,
                                  cnt_attr=cnt_attr,
                                  pol_attr=pol_attr,
                                  search_radius=idw_searchradius,
                                  power=idw_power)
    
    kriging_df, var = Kriging(track=data_gdf,
                              tracksegs_gdf=seg_gdf,
                              agg_attr=agg_attr,
                              cnt_attr=cnt_attr,
                              pol_attr=pol_attr,
    ...
    all_dfs = voronoi_df.merge(idw_df, on=agg_attr).merge(kriging_df, on=agg_attr)

    return all_dfs

all_interpolation = All(ngapipi_gdf_no2, Line_string_centroids)
...
all_interpolation.plot(ax= ax1, x = 'seg_group', y = 'voronoi_no2')
all_interpolation.plot(ax = ax2,y = 'idw_no2')
all_interpolation.plot(ax = ax3, y = 'krig_no2')


# In[156]:


all_interpolation.plot(y = 'voronoi_no2')


# In[155]:


all_interpolation.head(5000).plot(y='idw_no2')


# In[161]:


all_interpolation.head(1000).plot(y='krig_no2')

print(all_interpolation['krig_no2'].max())
print(all_interpolation['krig_no2'].min())      
...
# Assuming 'match_xy_tuple' is a column with tuples (lat, long)
# Extract lat and long from the tuple
ngapipi_gdf['lat'] = ngapipi_gdf['match_xy_tuple'].apply(lambda x: x[1])
ngapipi_gdf['long'] = ngapipi_gdf['match_xy_tuple'].apply(lambda x: x[0])

# Create a new geometry column based on the lat and long
geometry = [Point(lon, lat) for lon, lat in zip(ngapipi_gdf['long'], ngapipi_gdf['lat'])]
ngapipi_gdf = gpd.GeoDataFrame(ngapipi_gdf, geometry=geometry)

# Ensure the CRS is in EPSG:4326 (WGS 84)
ngapipi_gdf = ngapipi_gdf.set_crs("EPSG:4326", allow_override=True)

# Create a Folium map centered at the mean lat and long
m = folium.Map(location=[ngapipi_gdf['lat'].mean(), ngapipi_gdf['long'].mean()], zoom_start=12)

# Add the points to the map
for _, row in ngapipi_gdf.iterrows():
